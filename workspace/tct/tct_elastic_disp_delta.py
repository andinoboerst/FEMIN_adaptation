...
class TCTDispDeltaExtract(TCTSimulation):
...
class TCTDispDeltaApply(TCTSimulation):
    ...
    def _solve_time_step(self) -> None:
        current_forces = self.calculate_forces(self.interface_nodes)
        predicted_u_delta = self.predictor.predict([current_forces - self.forces_prev])[0]
        self.forces_prev = current_forces
