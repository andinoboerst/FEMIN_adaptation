...
def tct_elastic_generate_u_interface(frequency: int = 1000):
    ...
    for step in progressbar(range(num_steps)):
        ...
        interface_node_forces = np.zeros(len(interface_nodes) * 3)
...
def tct_elastic_apply_u_interface(predictor, frequency: int = 1000):
    ...
    for step in progressbar(range(num_steps)):
        ...
        interface_node_forces = np.zeros(len(top_boundary_nodes) * 3)
...
def tct_elastic_predictor_error_comparison(predictor, frequency: int = 1000):
    ...
    for step in progressbar(range(num_steps)):
        ...
        interface_node_forces_pred = np.zeros(len(top_boundary_nodes_pred) * 3)
