...
# 5. Variational Formulation (Linear Elasticity)
def epsilon(u):
...
displacement_data = []
pyvista_grid = pyvista.UnstructuredGrid(*dolfinx.plot.vtk_mesh(V))


xdmf_filename = "workspace/tct_results.xdmf" # Single file name for time series
with XDMFFile(mesh.comm, xdmf_filename, "w") as xdmffile:
    xdmffile.write_mesh(mesh) # Write mesh ONCE, outside the loop (for time series, mesh typically doesn't change topology)
    ...
    for step in progressbar(range(num_steps)):
        ...
        if step % 100 == 0:
            xdmffile.write_function(u_k, time) # Write displacement, including time
            # xdmffile.write_function(velocity_k, time) # Write velocity, including time
